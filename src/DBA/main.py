...
if __name__ == '__main__':
    ...
    if params_loaded['type'] == config.TYPE_LOAN:
        ...
        helper.load_data()
    elif params_loaded['type'] == config.TYPE_FASHION:
        helper = ImageHelper(current_time=current_time, params=params_loaded,
                             name=params_loaded.get('name', 'fashion'))
        helper.load_data()
    else:
        helper = None
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        if helper.params['aggregation_methods'] == config.AGGR_MEAN:
            is_updated = helper.fed_avg(helper.target_model, updates)
            num_oracle_calls = 1
            ...
            is_updated = helper.krum(helper.target_model, updates, f=helper.params['krum_f'])
            num_oracle_calls = 1
        
        elif helper.params['aggregation_methods'] == config.AGGR_TRIIMMEDMEAN:
            is_updated = helper.trimmed_mean(helper.target_model, updates, beta=helper.params['trim_beta'])
            num_oracle_calls = 1

        elif helper.params['aggregation_methods'] == config.AGGR_BULYAN_KRUM:
            is_updated = helper.bulyan_krum(helper.target_model, updates, f=helper.params['krum_f'])
            num_oracle_calls = 1
    
        elif helper.params['aggregation_methods'] == config.AGGR_BULYAN_TRIM:
            is_updated = helper.bulyan_trimmed_mean(helper.target_model, updates, f=helper.params['krum_f'])
            num_oracle_calls = 1

        elif helper.params['aggregation_methods'] == config.AGGR_FILTERL2:
            is_updated = helper.filterl2(helper.target_model, updates, sigma=helper.params['fliter_l2_sigma'], expansion=20, itv=None)
            num_oracle_calls = 1
        
        elif helper.params['aggregation_methods'] == config.AGGR_EXNOREGRET:
            is_updated = helper.ex_noregret(helper.target_model, updates, eps=1./5, sigma=helper.params['fliter_l2_sigma'], expansion=20, itv=1000)
            num_oracle_calls = 1
        
        elif helper.params['aggregation_methods'] == config.AGGR_MEDIAN:
            is_updated = helper.median(helper.target_model, updates)
            num_oracle_calls = 1
